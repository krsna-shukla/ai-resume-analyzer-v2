...
function App() {
  ...
  const clearFile = useCallback(() => {
    ...
    setAnalysis(null);
    setError("");
    setJobTitle("");            // optionally clear the job title too
  ...
  const uploadResume = async () => {
    ...
    try {
      const res = await axios.post(API_URL, formData, { timeout: 60000 });
      ...
      // ─── Compute Required + Missing for Job ───────────────
  ...
  const Bullet = ({ accent, index, children }) => (
    ...
    >
      {index != null && (
        <span style={{ color: T.primary, fontWeight: 700, minWidth: 18 }}>
          {index}.
        </span>
      )}
      <span>{children}</span>
    ...
    >
      ...
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className="skeleton"
          style={{ height: 13, width: `${85 - i * 8}%`, marginBottom: 12 }}
        />
      ))}
    </div>
  );
    ...
    >
      ...
      >
        ...
        <div className="fade-up" style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          >
            Instant ATS scoring, skill-gap analysis, and tailored AI suggestions to land more
            interviews.
          </p>

          {/* ─── Job Title Input ─────────────────────────────── */}
          <div style={{ margin: "20px auto", maxWidth: 460, textAlign: "left" }}>
            <label
              style={{
                display: "block",
                marginBottom: 6,
          ...
          <label
            ...
            style={{
              display: "block",
              padding: "38px 24px",
              background: dragOver ? "rgba(99,102,241,0.08)" : T.inner,
          ...
          >
            ...
            <p style={{ color: T.muted, margin: "6px 0 0", fontSize: 13.5 }}>
          ...
          {error && (
            <div role="alert" style={{ color: T.danger, fontSize: 14, marginTop: 14, fontWeight: 500 }}>
              ⚠ {error}
            </div>
          )}
          ...
          <div>
            <button
              ...
              style={{
                ...
                maxWidth: 460,
                fontFamily: T.font,
                boxShadow: loading || !file ? "none" : "0 8px 30px rgba(99,102,241,0.4)",
          ...
          {analysis && !loading && (
            <div className="fade-up">
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = T.borderHover;
                  e.currentTarget.style.background = T.inner;
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = T.border;
        ...
        {loading && (
          <div className="results-grid">
            ...
            <Skeleton lines={4} />
            <Skeleton lines={4} />
            <Skeleton lines={3} className="span-full" />
          </div>
        )}
        ...
        {analysis && !loading && (
          <div ref={reportRef} className="results-grid" style={{ padding: 4 }}>
            <Card delay={0.05}>
              ...
              <p style={{ textAlign: "center", color: scoreColor, fontWeight: 600, fontSize: 16, margin: "0 0 20px" }}>
                {grade}
              </p>
              <div style={{ padding: 16, background: T.inner, borderRadius: 14, border: `1px solid ${T.border}` }}>
                <h3 style={{ fontSize: 12, fontWeight: 600, margin: "0 0 8px", color: T.muted, letterSpacing: "0.05em" }}>
                  RESUME SUMMARY
                </h3>
                <p style={{ fontSize: 14, lineHeight: 1.6, margin: 0, color: T.textSoft }}>
            ...
            <Card delay={0.12}>
              <CardTitle icon="✅" count={analysis.skillsFound.length}>
                Skills Found
              </CardTitle>
              <div>
                {analysis.skillsFound.length > 0 ? (
                  analysis.skillsFound.map((s, i) => <Chip key={i} tone="success">{s}</Chip>)
                ) : (
            ...
            <Card delay={0.19}>
              <CardTitle icon="🎯" count={analysis.missingSkills.length}>
                Missing Skills
              </CardTitle>
              <div>
                {analysis.missingSkills.length > 0 ? (
                  analysis.missingSkills.map((s, i) => <Chip key={i} tone="danger">{s}</Chip>)
                ) : (
                  <p style={{ color: T.muted, fontSize: 14, margin: 0 }}>None detected.</p>
                )}
              </div>
            ...
            {/* ─── New: Required Skills ───────────────────────────── */}
            <Card delay={0.26}>
              <CardTitle icon="📋" count={analysis.requiredSkills?.length}>
                Required Skills
              </CardTitle>
              <div>
                {analysis.requiredSkills && analysis.requiredSkills.length > 0 ? (
                  analysis.requiredSkills.map((s, i) => <Chip key={i} tone="success">{s}</Chip>)
                ) : (
                  <p style={{ color: T.muted, fontSize: 14, margin: 0 }}>
                    No mapping for "{jobTitle.trim()}"
                  </p>
                )}
              </div>
            </Card>

            <Card className="span-full" delay={0.38}>
              <CardTitle icon="💡" count={analysis.suggestions.length}>
                AI Suggestions
              </CardTitle>
              {analysis.suggestions.length > 0 ? (
                analysis.suggestions.map((s, i) => (
                  <Bullet key={i} accent={T.primary} index={i + 1}>
                    {s}
                  </Bullet>
                ))
              ) : (
            ...
            <Card className="span-half-l" delay={0.44}>
              <CardTitle icon="⭐" count={analysis.strengths.length}>
                Strengths
              </CardTitle>
              {analysis.strengths.length > 0 ? (
                analysis.strengths.map((s, i) => (
                  <Bullet key={i} accent={T.success}>{s}</Bullet>
                ))
              ) : (
                <p style={{ color: T.muted, fontSize: 14, margin: 0 }}>Nothing to show here.</p>
              )}
            ...
            <Card className="span-half-r" delay={0.50}>
              <CardTitle icon="⚠️" count={analysis.weaknesses.length}>
                Weaknesses
              </CardTitle>
              {analysis.weaknesses.length > 0 ? (
                analysis.weaknesses.map((s, i) => (
                  <Bullet key={i} accent={T.warning}>{s}</Bullet>
                ))
              ) : (
                <p style={{ color: T.muted, fontSize: 14, margin: 0 }}>Nothing to show here.</p>
              )}
